...
shinyServer(function(input, output) {
  querydata <- reactive({
    queryExpression <- Neel_cell_line_per_gene_normalized_log2_FPKM_Allgene[rownames(Neel_cell_line_per_gene_normalized_log2_FPKM_Allgene) %in% input$geneExpressionQuery,]
    queryExpression <- queryExpression[,-c(1,2,3)]
    queryzGARP<-Cell_line_zGARP_Allgene[rownames(Cell_line_zGARP_Allgene) %in% input$geneExpressionQuery,]
  ...
  observe({
    if(input$plotCommand == TRUE) {
      output$expressionPlot<-renderPlot({
        ...
        Genedata<-as.data.frame(t(data.matrix(Neel_cell_line_per_gene_normalized_log2_FPKM_Allgene[gene,-c(1,2,3)])))
        Genedata$histology<-Nell_cell_line_subtype[rownames(Genedata),"histology"]
