library(shiny)
library(ggplot2)
#setwd("/Users/liz6/Box Sync/WCRC/Lee-Oesterreich-lab-personnel/Current/Vaciry/Vaciry/Bioinformatical analysis/Shiny/E2_regulation/")
MCF7_1<-read.csv("SO_MCF7.csv",sep=",",header=T)
MCF7_2<-read.csv("Liu_CPM.csv",sep=",",header=T)
...
T47D_3<-read.csv("MB_T47D.csv",sep=",",header=T)
...
shinyServer(function(input, output) {
  ...
  output$MCF7_2 <- renderPlot({
    ...
    df["Veh", "E2_Veh"] = "Veh"
    df["Veh", "meanTPM"] = mean(as.vector(as.matrix(MCF7_2[row, 2:9])))
    df["Veh", "sdTPM"] = sd(as.vector(as.matrix(MCF7_2[row, 2:9])))
    
    df["E2", "E2_Veh"] = "E2"
    df["E2", "meanTPM"] = mean(as.vector(as.matrix(MCF7_2[row, 10:17])))
    df["E2", "sdTPM"] = sd(as.vector(as.matrix(MCF7_2[row, 10:17])))
  ...
  output$T47D_3 <- renderPlot({
    ...
    df["Veh", "E2_Veh"] = "Veh"
    df["Veh", "meanTPM"] = mean(as.vector(as.matrix(T47D_3[row, 2:4])))
    df["Veh", "sdTPM"] = sd(as.vector(as.matrix(T47D_3[row, 2:4])))
    
    df["E2", "E2_Veh"] = "E2"
    df["E2", "meanTPM"] = mean(as.vector(as.matrix(T47D_3[row, 5:7])))
    df["E2", "sdTPM"] = sd(as.vector(as.matrix(T47D_3[row, 5:7])))
