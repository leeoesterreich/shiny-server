...
shinyServer(function(input, output) {
  ...
  output$all <-renderPlot({
    ...
    ggplot(subset(mdat_pairs, (external_gene_name %in% gene)), aes(x=group, y=log2CPM, color=metSite,group=pair))  +
      ...
      #scale_color_manual(values= c("purple4", "red4","darkorange", "blue")) +
  ...
  output$ov.sb <-renderPlot({
    ...
    boxplot(ovaries.MET.IDC$log2CPM,ovaries.MET.ILC$log2CPM,main= paste(gene,"expression in Ovarian Mets(IDC vs ILC)"),cex.main=1.3,outline = TRUE,pars = list(boxwex = 0.8, staplewex = 0.5, outwex = 0.5),
            log = "",
            xlab=paste("Non-Paired Wilcoxon.test p value=",format(w2$p.value, digits = 2)),ylab="log2 TMM-normalized CPM ",
  ...
  output$br.er <-renderPlot({
    ...
    boxplot(Brain.MET.ERpos$log2CPM,Brain.MET.ERneg$log2CPM,main= paste(gene,"expression in Brain Mets(ERpos vs ERneg)"),cex.main=1.3,outline = TRUE,pars = list(boxwex = 0.8, staplewex = 0.5, outwex = 0.5),
            log = "",
            xlab=paste("Non-Paired Wilcoxon.test p value=",format(w3$p.value, digits=2)),ylab="log2 TMM-normalized CPM ",
